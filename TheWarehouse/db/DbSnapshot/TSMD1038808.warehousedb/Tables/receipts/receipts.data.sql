 ...
 *
 * Purpose:
 *   To recreate the Data in receipts Table
 * Change History:
 *
 */
...
SET @SQL = 'SELECT * INTO [].[receipts_' + @Revision + '_' + @TimeStamp + '] FROM [].[receipts]'
    
    EXEC sp_executesql @SQL

IF (@@ROWCOUNT = 0 AND (SELECT COUNT(*) FROM [].[receipts]) > 0)
OR (@@ERROR > 0)
BEGIN
...
ELSE
BEGIN
    SET @StatusMessage = 'All existing data was copied into the table [].[receipts_' + @Revision + '_' + @TimeStamp + ']'
    PRINT @StatusMessage
END
--#endregion
...
DBCC CHECKIDENT ('[].[receipts]', RESEED, 1)
IF @@ERROR > 0
BEGIN
...
SELECT 
	...
	[EmployeeId],
	[timestamp]
FROM OPENXML(@DocumentHandle, 'receiptsRows/receiptsRow', 2) WITH ( 
	[idReceipts] INT,
	[Store] VARCHAR(135),
...
BEGIN
	...
	PRINT 'Transaction for [].[receipts] Rolled Back'
END
GO
