 ...
 *
 * Purpose:
 *   To recreate the Data in employees Table
 * Change History:
 *
 */
...
SET @SQL = 'SELECT * INTO [].[employees_' + @Revision + '_' + @TimeStamp + '] FROM [].[employees]'
    
    EXEC sp_executesql @SQL

IF (@@ROWCOUNT = 0 AND (SELECT COUNT(*) FROM [].[employees]) > 0)
OR (@@ERROR > 0)
BEGIN
...
ELSE
BEGIN
    SET @StatusMessage = 'All existing data was copied into the table [].[employees_' + @Revision + '_' + @TimeStamp + ']'
    PRINT @StatusMessage
END
--#endregion
...
SELECT 
	...
	[created_ts],
	[updated_ts]
FROM OPENXML(@DocumentHandle, 'employeesRows/employeesRow', 2) WITH ( 
	[idEmployees] INT,
...
BEGIN
	...
	PRINT 'Transaction for [].[employees] Rolled Back'
END
GO
