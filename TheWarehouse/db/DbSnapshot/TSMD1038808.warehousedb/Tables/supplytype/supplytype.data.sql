 ...
 *
 * Purpose:
 *   To recreate the Data in supplytype Table
 * Change History:
 *
 */
...
SET @SQL = 'SELECT * INTO [].[supplytype_' + @Revision + '_' + @TimeStamp + '] FROM [].[supplytype]'
    
    EXEC sp_executesql @SQL

IF (@@ROWCOUNT = 0 AND (SELECT COUNT(*) FROM [].[supplytype]) > 0)
OR (@@ERROR > 0)
BEGIN
...
ELSE
BEGIN
    SET @StatusMessage = 'All existing data was copied into the table [].[supplytype_' + @Revision + '_' + @TimeStamp + ']'
    PRINT @StatusMessage
END
--#endregion
...
SELECT 
	[Name]
FROM OPENXML(@DocumentHandle, 'supplytypeRows/supplytypeRow', 2) WITH ( 
	[Name] VARCHAR(135)
)
...
BEGIN
	...
	PRINT 'Transaction for [].[supplytype] Rolled Back'
END
GO
