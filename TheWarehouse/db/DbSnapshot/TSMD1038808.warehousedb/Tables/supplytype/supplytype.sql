...
IF (OBJECT_ID(N'[].[supplytype]')) IS NOT NULL
BEGIN
    IF EXISTS ( SELECT * FROM [].[supplytype] )
    BEGIN
        DECLARE @newName NVARCHAR(128); SET @newName = N'[].[supplytype_' + CONVERT(NVARCHAR(128), NEWID()) + N'_MAY_DELETE]'
        EXEC ('SELECT * INTO ' + @newName + ' FROM [].[supplytype]')
...
IF OBJECT_ID(N'[].[supplytype]') IS NULL
BEGIN
