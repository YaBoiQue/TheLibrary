 ...
 *
 * Purpose:
 *   To recreate the Data in transactionitems Table
 * Change History:
 *
 */
...
SET @SQL = 'SELECT * INTO [].[transactionitems_' + @Revision + '_' + @TimeStamp + '] FROM [].[transactionitems]'
    
    EXEC sp_executesql @SQL

IF (@@ROWCOUNT = 0 AND (SELECT COUNT(*) FROM [].[transactionitems]) > 0)
OR (@@ERROR > 0)
BEGIN
...
ELSE
BEGIN
    SET @StatusMessage = 'All existing data was copied into the table [].[transactionitems_' + @Revision + '_' + @TimeStamp + ']'
    PRINT @StatusMessage
END
--#endregion
...
SELECT 
	...
	[Price],
	[timestamp]
FROM OPENXML(@DocumentHandle, 'transactionitemsRows/transactionitemsRow', 2) WITH ( 
...
BEGIN
	...
	PRINT 'Transaction for [].[transactionitems] Rolled Back'
END
GO
