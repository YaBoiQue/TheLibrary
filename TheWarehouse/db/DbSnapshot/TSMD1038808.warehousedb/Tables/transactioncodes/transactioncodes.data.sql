 ...
 *
 * Purpose:
 *   To recreate the Data in transactioncodes Table
 * Change History:
 *
 */
...
SET @SQL = 'SELECT * INTO [].[transactioncodes_' + @Revision + '_' + @TimeStamp + '] FROM [].[transactioncodes]'
    
    EXEC sp_executesql @SQL

IF (@@ROWCOUNT = 0 AND (SELECT COUNT(*) FROM [].[transactioncodes]) > 0)
OR (@@ERROR > 0)
BEGIN
...
ELSE
BEGIN
    SET @StatusMessage = 'All existing data was copied into the table [].[transactioncodes_' + @Revision + '_' + @TimeStamp + ']'
    PRINT @StatusMessage
END
--#endregion
...
SELECT 
	[Name],
	[Description]
FROM OPENXML(@DocumentHandle, 'transactioncodesRows/transactioncodesRow', 2) WITH ( 
	[Name] VARCHAR(135),
...
BEGIN
	...
	PRINT 'Transaction for [].[transactioncodes] Rolled Back'
END
GO
